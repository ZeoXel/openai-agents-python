...
**A:** 如果看到 "Request timed out" 错误：

**原因：**
- API 响应较慢（GPT-5 处理复杂请求需要时间）
- 网络连接不稳定
- API 端点临时负载较高

**解决方案：**
1. **等待 30 秒后重试**（最常见解决方案）
2. **直接重新输入相同内容**
3. **检查网络连接**
4. **运行连接测试**：
   ```bash
